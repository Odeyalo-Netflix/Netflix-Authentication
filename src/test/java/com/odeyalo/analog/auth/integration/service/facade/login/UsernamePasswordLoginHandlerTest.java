...
class UsernamePasswordLoginHandlerTest {
    ...
    private static final String CORRECT_USER_PASSWORD = "123";
    ...
    @BeforeAll
    void setUp() {
        User user = TestUtils.buildUser(1, EXISTED_USER_EMAIL, EXISTED_USER_NICKNAME, this.encoder.encode(CORRECT_USER_PASSWORD), false, AuthProvider.LOCAL, true, "", Role.USER);
        this.userRepository.save(user);
    }
    @Test
    @DisplayName("Login with right nickname and password")
    void loginExistedUserWithRightNicknameAndPassword() {
        User userToLogin = User.builder().nickname(EXISTED_USER_NICKNAME).password(CORRECT_USER_PASSWORD).build();
        ...
        assertNotEquals(CORRECT_USER_PASSWORD, user.getPassword());
    }
    @Test
    @DisplayName("Login with right nickname and password")
    ...
    void loginExistedUserWithWrongNicknameAndRightPassword() {
        User userToLogin = User.builder().nickname(NOT_EXISTED_USER_NICKNAME).password(CORRECT_USER_PASSWORD).build();
        assertThrows(UserNotExistException.class, () -> this.loginHandler.login(userToLogin));
    }
    @Test
